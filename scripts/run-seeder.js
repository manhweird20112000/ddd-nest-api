...
async function run() {
  const [moduleName, fileName] = process.argv.slice(2);
  ...
  const seederPath = path.join(
    __dirname,
    `../dist/modules/${moduleName}/infrastructure/persistence/seeders/${fileName}`,
  );
  ...
  console.log(`Seeder "${fileName}" from module "${moduleName}" executed.`);
}
